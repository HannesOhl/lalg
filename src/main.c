		...
		*/
		...
		for(int32_t y = y_min; y <= y_max; y++) {
			...
			if (is_point_in_triangle(p, v1, v2, v3)) {
				...
				float denominator = (v2.y - v3.y)*(v1.x - v3.x)
					          + (v3.x - v2.x)*(v1.y - v3.y);
				if (fabsf(denominator) < EPS) return;

				float a = ( (v2.y - v3.y) * (x   - v3.x)
					  + (v3.x - v2.x) * (p.y - v3.y) ) / denominator;
				float c = 1.0f - a;
				float z_inv = 1.0f / t.v1.z * c
					    + 1.0f / t.v2.z * a;
				float z = 1.0f / z_inv;

				size_t idx = x + y * (size_t) SCREEN_WIDTH;
...
*/
		...
		*/
		...
		text_render(string_format("frame time = %.2f ms, FPS = %.2f (256 samples);"
					"lines drawn = %zuk triangles drawn = %zu\n",
