// TODO: add aabb to model, remove min/max from mesh (-> mesh const then).
...
Model model_assemble(const void* asset, const Texture* asset_texture) {
	...
	float x_min = 0.0f; float y_min = 0.0f; float z_min = 0.0f;
	float x_max = 0.0f; float y_max = 0.0f; float z_max = 0.0f;
	for (int i = res.mesh->v_count - 1; i >= 0; i--) {
...
void aabb_render(V3f min, V3f max, uint32_t* buffer, Camera camera) {

	V3f min_ground_min = { .x = min.x, .y = min.y, .z = min.z};
	V3f min_ground_max = { .x = min.x, .y = min.y, .z = max.z};
	V3f max_ground_min = { .x = max.x, .y = min.y, .z = min.z};
	V3f max_ground_max = { .x = max.x, .y = min.y, .z = max.z};

	V3f min_sky_min = { .x = min.x, .y = max.y, .z = min.z};
	V3f min_sky_max = { .x = min.x, .y = max.y, .z = max.z};
	V3f max_sky_min = { .x = max.x, .y = max.y, .z = min.z};
	V3f max_sky_max = { .x = max.x, .y = max.y, .z = max.z};
...
void event_loop(SDLContext* ctx, uint32_t* buffer, Camera camera, Model* models) {
	...
	while (running) {
		...
		*/
		if (state.abb) aabb_render(models[3].mesh->min, models[3].mesh->max, buffer, camera);
...
int main(void) {
	...
	models[3] = model_assemble(&asset_messa, &texture_messa);
